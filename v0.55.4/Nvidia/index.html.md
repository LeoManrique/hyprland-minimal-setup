...
1. Try changing the primary GPU [with the AQ\_DRM\_DEVICES environment variable](../Configuring/Advanced-and-Cool/Multi-GPU/index.html#telling-hyprland-which-gpu-to-use).
...
[Loading the Nvidia modules early](index.html#early-kms-modeset-and-fbdev) may cause resuming from hibernation to not work anymore (i.e. the system will just boot instead of resuming).
If you have issues with that, try disabling early KMS.
