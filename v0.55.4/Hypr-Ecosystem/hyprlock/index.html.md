...
Hyprlock uses the following types in addition to [Hyprland’s variable types](https://wiki.hypr.land/Configuring/Basics/Variables#variable-types).
...
Available animations can be found in the [animation tree](index.html#animation-tree).
The optional `STYLE` parameter for the `animation` keyword is currently unused by hyprlock.

Check out Hyprland’s [animation documentation](https://wiki.hypr.land/Configuring/Advanced-and-Cool/Animations) for more information.
...
The [bind flag](../../Configuring/Basics/Binds/index.html#bind-flags) `l` can be used to allow specific hyprland keybinds to also work while hyprlock is active (e.g. brightness/volume/media control).
...
See [Variables/#blur](../../Configuring/Basics/Variables/index.html#blur).
...
| `check_text` | Text rendered when waiting for the authentication result. | str | *empty* |
...
`placeholder_text` and `fail_text` both support [variable substitution](index.html#variable-substitution).
...
The `text` option supports [variable substitution](index.html#variable-substitution) and launching shell commands.  
For example:
...
[hypridle](../hypridle/index.html "hypridle")[xdg-desktop-portal-hyprland](../xdg-desktop-portal-hyprland/index.html "xdg-desktop-portal-hyprland")