...
More info in [Notes](index.html#notes)
...
If you want to *negate* a RegEx, as in pass only when the RegEx *fails*, you
...
| group | string | Sets window group properties. See [group options](index.html#group-window-rule-options) below. |
...
| no\_follow\_mouse | boolean | Prevents the window from being focused when the mouse moves over it when `input.follow_mouse=1` is set. |
...
| scroll\_mouse | number | Forces the window to override `input.scroll_factor`. |
| scroll\_touchpad | number | Forces the window to override `input.touchpad.scroll_factor`. |
...
Effects marked as *Dynamic* are reevaluated whenever the matching property
...
[Dispatchers](../Dispatchers/index.html "Dispatchers")[Workspace Rules](../Workspace-Rules/index.html "Workspace Rules")