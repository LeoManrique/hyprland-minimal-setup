...
`leaf` is scope of the animation. See [Animation tree](index.html#animation-tree)

`enabled` use `true` to enable, `false` to disable. *Note:* if it’s `false`, you
can omit further args.

`speed` is the amount of ds (1ds = 100ms) the animation will take. For example `speed = 1` = 100ms

`bezier` / `spring` is the curve name, see [curves](index.html#curves).

`style` (optional) is the animation style. See [Animation tree](index.html#animation-tree)
...
[Gestures](../Gestures/index.html "Gestures")