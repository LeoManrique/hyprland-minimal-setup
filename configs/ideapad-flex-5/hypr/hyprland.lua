...
--------------------------------------------------------------------------------
-- AMD Radeon (Ryzen 5 5500U iGPU) — no vendor env needed; the two NVIDIA lines
-- (LIBVA_DRIVER_NAME / __GLX_VENDOR_LIBRARY_NAME) from the desktop are dropped.
...
-- Laptop: single internal panel eDP-1 (1920x1080) at integer scale 1 — crisp,
-- and no GTK3/webview fractional-scale blur. The catch-all (first, lower prio)
-- auto-places an external HDMI-A-1 display to the right when one is plugged in.
...
hl.config({
  ...
  input = {
    ...
    sensitivity = 0.25,           -- faster cursor movement (-1.0 to 1.0)
    touchpad = {
...
hl.on("hyprland.start", function()
  ...
  -- NB: do NOT prefix this with "env GDK_SCALE=2" — nwg-dock fork/execs the apps
  -- you click, so they'd inherit GDK_SCALE and render double-size. (It sharpened
  -- the dock's own icons under fractional scale, but the leak isn't worth it.)
  hl.exec_cmd("nwg-dock-hyprland -r -i 44 -p bottom -a center -mb 8 -o eDP-1 -x -nolauncher")
  -- Idle / lock daemon
...
hl.bind(mod .. " + CTRL + SHIFT + Q", hl.dsp.exec_cmd("~/.config/hypr/scripts/close-workspace"))  -- close ALL windows on the active workspace
